...
export class ResetPasswordComponent implements OnInit { 
  ...
  verifyOTP(otp): void{
    this.verificationService.verifyOtp({ otp, email:this.email}).subscribe((response) => {
      if (response['body'].status) {
  ...
  updatePassword(password): void{ 
    this.forgetPassword.updatePassword({email:this.email,password}).subscribe((response) => {
      if (response['body'].status){
        alert(response['body'].message);
